...
```prompt
옵션을 바꿀수 없을가? 예를들면 rov의 무게라던지  주사기의 용량이라던지 등이
있어야하지않을가?
...
uv run python main.py --mass 4.0 --syringe 120 --pool-depth 3.5  
 uv run python main.py --mass 2.5 --syringe 60 --cd 1.5 --rho 1000
...
┌──────────────┬────────┬──────────────────┐  
 │ 옵션 │ 기본값 │ 설명 │
├──────────────┼────────┼──────────────────┤  
 │ --mass │ 3.0 │ Float 무게 (kg) │  
 ├──────────────┼────────┼──────────────────┤
│ --radius │ 0.08 │ Float 반지름 (m) │
├──────────────┼────────┼──────────────────┤  
 │ --cd │ 1.2 │ 항력 계수 │
├──────────────┼────────┼──────────────────┤  
 │ --syringe │ 80.0 │ 주사기 용량 (mL) │  
 ├──────────────┼────────┼──────────────────┤  
 │ --pool-depth │ 4.0 │ 풀 깊이 (m) │
├──────────────┼────────┼──────────────────┤  
 │ --rho │ 1025.0 │ 물 밀도 (kg/m^3) │  
 └──────────────┴────────┴──────────────────┘
...
```
시뮬레이터가 끝나도 뭐가 숫자가 움직이는데?
```

```
가속도도 고민하고 진행중이야? if yes , explain code
```

```
그리고 바닥과 아이스에 닿으면 실격이라 인풋데이터가 잘못되면 실패/성공이 나오면 좋겟어
```
